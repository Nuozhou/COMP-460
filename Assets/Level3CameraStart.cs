...
public class Level3CameraStart : MonoBehaviour {
	...
	void Start () {
		...
		Camera.main.transform.position = new Vector3 (0, 240, -10);
	...
	IEnumerator WaitAndDirectToPlayer() {
		while (Camera.main.orthographicSize > 20f) {
			...
			Camera.main.transform.position = new Vector3 (Camera.main.transform.position.x - 0.05f, Camera.main.transform.position.y - 0.5f, -10);
		}
